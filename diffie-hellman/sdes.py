from utils import permute, xor_bits, bits_to_int, int_to_bits, text_to_blocks, blocks_to_text
...
def decrypt_block(ciphertext_block, key10):
...
def encrypt_text(text, key10): #szyfrowanie calego tekstu
    blocks = text_to_blocks(text)
    return [encrypt_block(block, key10) for block in blocks]


def decrypt_text(cipher_blocks, key10): #odszyfrowywanie calego kryptogramu
    plain_blocks = [decrypt_block(block, key10) for block in cipher_blocks]
